...
namespace QboxNext.Qserver.DataStore
{
    /// <remarks>
    /// TODO: should inject stream/reader so its easier to mock.
    /// </remarks>
    [NonParallelizable]
    public class KWhStorageTest
    {
        ...
        public void FindPreviousWorksCorrectlyWithUntruncatedTimeTest()
        {
            ...
            using (var storage = new kWhStorage(_options, _dataStorageContext))
            {
                ...
                Record record = storage.FindPrevious(baseTimestamp.AddMinutes(1));
        ...
        {
            // act
            using (var target = new kWhStorage(_options, _dataStorageContext))
            {
                try
                {
                    List<SeriesValue> values = SeriesValueListBuilder.BuildSeries(DateTime.Today.AddDays(-7), DateTime.Today,
                        SeriesResolution.Day);
                    bool result = target.GetRecords(DateTime.Today.AddDays(-7), DateTime.Today, Unit.kWh, values, false);
                    // assert
        ...
        public void GetRecordsShouldReturnWithinReasonableTimeWhenMeasurementsAreMissingfromTheFileTest()
        {
            ...
            for (var i = 0; i < 2; i++)
            {
                using (var target = new kWhStorage(_options, _dataStorageContext))
                {
                    ...
                    watch.Start();
                    List<SeriesValue> values = SeriesValueListBuilder.BuildSeries(DateTime.Today.AddDays(-30), DateTime.Today,
                        SeriesResolution.Day);

                    bool result = target.GetRecords(DateTime.Today.AddDays(-30), DateTime.Today, Unit.kWh, values, false);
                    watch.Stop();
        ...
        public void GetSumTest()
        {
            using (var target = new kWhStorage(_options, _dataStorageContext))
            {
                ...
                DateTime begin = DateTime.Today.AddDays((DateTime.Today.Day - 1) * -1);
                var end = new DateTime(DateTime.Now.Year, DateTime.Now.Month,
                    DateTime.DaysInMonth(DateTime.Now.Year, DateTime.Now.Month));

                // act
                decimal result = target.Sum(begin, end, Unit.kWh);
        ...
        {
            // arrange
            DateTime measureTime = DateTime.Now.AddMinutes(-15);
            Record actual = null;
            using (var target = new kWhStorage(_options, _dataStorageContext))
            {
                target.SetValue(measureTime, 50, 25, 15);
        ...
        {
            double intermediate = Math.Log10(5);
            ushort quality = Convert.ToUInt16(intermediate * 10000);
        ...
        {
            // arrange
            int counterId = _dataStorageContext.CounterId = 15;
            ...
            // assert
            Assert.IsTrue(File.Exists(BaseDir + $"\\Qbox_Serial\\Serial_{counterId:00000000}.qbx"));
        }
        ...
        {
            // arrange
            DateTime measureTime = DateTime.Now.AddDays(-10);

            DateTime secondMeasureTime = DateTime.Now.AddDays(-1);
        ...
        public void SetValueShouldStoreTheRunningTotalsInTheFileTest()
        {
            ...
            using (var storageProvider = new kWhStorage(_options, _dataStorageContext))
            {
                // act
                for (var i = 0; i < 10; i++)
                {
            ...
            {
                // assert
                string fileName = BaseDir + $"\\Qbox_12-13-001-075\\12-13-001-075_{counterId:00000000}.qbx";
                Assert.IsTrue(File.Exists(fileName));
                for (var i = 0; i < 10; i++)
                {
                    Record actual = storageProvider.GetValue(time.AddMinutes(i));
        ...
        public void SetValueShouldWriteTheRecordToFileAtOffsetTest()
        {
            ...
            Assert.IsTrue(File.Exists(BaseDir + $"\\Qbox_Serial\\Serial_{counterId:00000000}.qbx"));
            const int offset = 32;
            using (var reader = new BinaryReader(File.OpenRead(BaseDir + $"\\Qbox_Serial\\Serial_{counterId:00000000}.qbx")))
            {
        ...
        public void SumTest()
        {
            ...
            using (var target = new kWhStorage(_options, _dataStorageContext))
            {
                ...
                Period period = PeriodBuilder.Yesterday();
                DateTime begin = period.From;
                DateTime end = period.To;
        ...
        public void WhenFirstSlotIsInvalidRestOfSlotsShouldBeFilledTest()
        {
            ...
            using (var storage = new kWhStorage(_options, _dataStorageContext))
            {
                ...
                Record record = storage.SetValue(lastZeroTimestamp, 0, 1000.0m, 0.21m);
                ...
                DateTime timestamp = firstActualTimestamp.Date.AddDays(1);
                ulong value = 100000;
                ...
                List<SeriesValue> slots = SeriesValueListBuilder.BuildSeries(startPeriod, endPeriod, SeriesResolution.Day);
                Assert.IsTrue(storage.GetRecords(startPeriod, endPeriod, Unit.kWh, slots, false));

                Assert.IsNull(slots[0].Value);
                foreach (SeriesValue slot in slots.Skip(1))
                {
        ...
        public void WhenFirstTimestampOfFirstSlotIsInvalidMeasurementNextValueShouldBeUsedTest()
        {
            ...
            using (var storage = new kWhStorage(_options, _dataStorageContext))
            {
                ...
                foreach (SeriesValue slot in slots)
                {
                    Assert.IsNotNull(slot.Value);
                }
            }
        ...
        public void WhenReferenceDateIsBeforeEndOfFileGetSeriesShouldReturnValue()
        {
            ...
            using (var storage = new kWhStorage(_options, _dataStorageContext))
            {
                DateTime baseTimestamp = DateTime.Now.Date;
                ...
                List<SeriesValue> slots = SeriesValueListBuilder.BuildSeries(baseTimestamp, baseTimestamp.AddDays(7), SeriesResolution.Day);
        ...
        public void WhenReinitializeIsCalledWithTimestampBeyondFileShouldNotThrowException()
        {
            ...
            using (var storage = new kWhStorage(_options, _dataStorageContext))
            {
                ...
                DateTime endOfFile = storage.EndOfFile;
        ...
        public void WhenSetValueIsCalledWith2000AsFormulaAndAPrecisionOf10000IsShouldWriteAHalfTest()
        {
            ...
            using (var target = new kWhStorage(_options, _dataStorageContext))
            {
                ...
                List<SeriesValue> series = SeriesValueListBuilder.BuildSeries(start.AddMinutes(-5), start, SeriesResolution.OneMinute);
        ...
        public void WhenSetValueIsCalledWithAnIntervalItShouldLevelTheValuesWrittenTest()
        {
            ...
            using (var target = new kWhStorage(_options, _dataStorageContext))
            {
                ...
                Assert.AreEqual(ulong.MaxValue, target.GetValue(start.AddMinutes(-8)).Raw);
                Assert.AreEqual(ulong.MaxValue, target.GetValue(start.AddMinutes(-1)).Raw);
                Assert.AreEqual(ulong.MaxValue, target.GetValue(start.AddMinutes(-9)).Raw);
        ...
        public void WhenSlotEndsAfterEndOfFileButBeforeReferenceDateGetSeriesShouldReturnValue()
        {
            ...
            using (var storage = new kWhStorage(_options, _dataStorageContext))
            {
                ...
                // This should still result in a bit of consumption.
                List<SeriesValue> slots = SeriesValueListBuilder.BuildSeries(baseTimestamp, baseTimestamp.AddMonths(1), SeriesResolution.Month);
        ...
        public void WhenSumIsCalledItCalculatesTheSameAsTheIndividualResolutionTest()
        {
            ...
            using (var target = new kWhStorage(_options, _dataStorageContext))
            {
                ...
                // assert
                Period thisWeek = PeriodBuilder.LastWeek();
                List<SeriesValue> thisWeekSeries = SeriesValueListBuilder.BuildSeries(thisWeek.From, thisWeek.To, SeriesResolution.Hour);
                target.GetRecords(thisWeek.From, thisWeek.To, Unit.M3, thisWeekSeries, false);
                decimal? thisWeekSeriesSum = thisWeekSeries.Sum(s => s.Value);

                decimal thisWeekSum = target.Sum(thisWeek.From, thisWeek.To, Unit.M3);
        ...
        public void WhenTheLastFiveMinutesAreRequestedFromTheFileItShouldCaluclateTheWeightForTheActualTimeGapTest()
        {
            ...
            using (var target = new kWhStorage(_options, _dataStorageContext))
            {
                ...
                target.GetValue(start);
                // assert
                List<SeriesValue> series = SeriesValueListBuilder.BuildSeries(DateTime.Today, DateTime.Now, SeriesResolution.FiveMinutes);
        ...
        public void WhenValueIsWrittenInLastSlotShouldStoreValueAndNotGrowFile()
        {
            ...
            using (var storage = new kWhStorage(_options, _dataStorageContext))
            {
                Record record = storage.GetValue(timestampAtEnd);
        ...
        public void WhenValueIsWrittenOneMinuteBeforeStartShouldThrowException()
        {
            ...
            Assert.Throws(typeof(InvalidOperationException), delegate
            {
                using (var storage = new kWhStorage(_options, _dataStorageContext))
                {
                    ...
                    DateTime timestamp = startOfFile.AddMinutes(-1);
        ...
        public void WhenValuesAreWrittenWithRunningTotalGetValueShouldReturnSameValues()
        {
            ...
            using (var storage = new kWhStorage(_options, _dataStorageContext))
            {
                for (var i = 0; i < 10; ++i)
                {
                    Record record = storage.GetValue(new DateTime(2013, 5, 7, 0, i, 0));
